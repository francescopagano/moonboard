...
pal <- viridis(5, option = "inferno", direction = -1)
sub_pal <- pal
...
coef_df_xgb = as.data.frame(importance_matrix_r)
coef_df_xgb

coef_df_xgb = coef_df_xgb[, c(1,5)]
coef_df_xgb


colnames(coef_df_xgb)[1] = "var"
colnames(coef_df_xgb)[2] = "coeff"
...
# 2. mean_rowband
mean_rowband_vars <- coef_df_xgb %>%
  filter(str_detect(var, "^mean_rowband"))

# 3. mean_colband
mean_colband_vars <- coef_df_xgb %>%
  filter(str_detect(var, "^mean_colband"))

# 4. Blocchi per tipo e solo quelli con "mean"
blocchi_6x4 <- coef_df_xgb %>% filter(str_detect(var, "^blocco6x4.*mean"))
blocchi_9x4 <- coef_df_xgb %>% filter(str_detect(var, "^blocco9x4.*mean"))
blocchi_6x2 <- coef_df_xgb %>% filter(str_detect(var, "^blocco6x2.*mean"))
blocchi_3x3 <- coef_df_xgb %>% filter(str_detect(var, "^blocco3x3.*mean"))

blocchi_6x3 <- coef_df_xgb %>% filter(str_detect(var, "^blocco6x3.*mean"))
blocchi_9x2 <- coef_df_xgb %>% filter(str_detect(var, "^blocco9x2.*mean"))
...
# prese$coeff[prese$coeff >= -0.00075 & prese$coeff <= 0.00075] <- NA
# mean_rowband_vars$coeff[mean_rowband_vars$coeff >= -0.00075 & mean_rowband_vars$coeff <= 0.00075] <- NA
# mean_colband_vars$coeff[mean_colband_vars$coeff >= -0.00075 & mean_colband_vars$coeff <= 0.00075] <- NA
# blocchi_6x4$coeff[blocchi_6x4$coeff >= -0.00075 & blocchi_6x4$coeff <= 0.00075] <- NA
# blocchi_9x4$coeff[blocchi_9x4$coeff >= -0.00075 & blocchi_9x4$coeff <= 0.00075] <- NA
# blocchi_6x2$coeff[blocchi_6x2$coeff >= -0.00075 & blocchi_6x2$coeff <= 0.00075] <- NA
# blocchi_3x3$coeff[blocchi_3x3$coeff >= -0.00075 & blocchi_3x3$coeff <= 0.00075] <- NA
# blocchi_6x3$coeff[blocchi_6x3$coeff >= -0.00075 & blocchi_6x3$coeff <= 0.00075] <- NA
# blocchi_9x2$coeff[blocchi_9x2$coeff >= -0.00075 & blocchi_9x2$coeff <= 0.00075] <- NA

# prese$coeff[prese$coeff == 0] <- NA
...
q_low  <- quantile(coef_df_xgb$coeff, 0.15)
q_high <- quantile(coef_df_xgb$coeff, 0.85)
...
p1 <- ggplot() +
  ...
  theme_void() +
  labs(title = "Intensità coefficienti delle prese - XGBoost") +
  theme(
    legend.position = "right",  # legenda a destra
    legend.title = element_text(size = 10),
    legend.text = element_text(size = 10),
...
p_blocchi_3x3 <- plot_blocchi(sf_blocchi_3x3, grid_sf, df_col, df_row,
                              title = "Blocchi 3x3 - XGBoost", fill_lab = "Coefficiente blocchi 3x3", sub_pal)
print(p_blocchi_3x3)

# Blocchi 6x4
p_blocchi_6x4 <- plot_blocchi(sf_blocchi_6x4, grid_sf, df_col, df_row,
                              title = "Blocchi 6x4 - XGBoost", fill_lab = "Coefficiente blocchi 6x4", sub_pal)
print(p_blocchi_6x4)

# Blocchi 6x2
p_blocchi_6x2 <- plot_blocchi(sf_blocchi_6x2, grid_sf, df_col, df_row,
                              title = "Blocchi 6x2 - XGBoost", fill_lab = "Coefficiente blocchi 6x2", sub_pal)
print(p_blocchi_6x2)

# Blocchi 9x4
p_blocchi_9x4 <- plot_blocchi(sf_blocchi_9x4, grid_sf, df_col, df_row,
                              title = "Blocchi 9x4 - XGBoost", fill_lab = "Coefficiente blocchi 9x4", sub_pal)
print(p_blocchi_9x4)

# Blocchi 6x2
p_blocchi_6x3 <- plot_blocchi(sf_blocchi_6x3, grid_sf, df_col, df_row,
                              title = "Blocchi 6x3 - XGBoost", fill_lab = "Coefficiente blocchi 6x3", sub_pal)
print(p_blocchi_6x3)

# Blocchi 9x2
p_blocchi_9x2 <- plot_blocchi(sf_blocchi_9x2, grid_sf, df_col, df_row,
                              title = "Blocchi 9x2 - XGBoost", fill_lab = "Coefficiente blocchi 9x2", sub_pal)
print(p_blocchi_9x2)


### VISUALIZZAZIONE INTEGRATA ----

blocchi_sf1 = sf_blocchi_6x3          # Blocchi 3x3 (puoi cambiare con sf_blocchi_6x4, ecc)
blocchi_sf2 = sf_blocchi_6x2
blocchi_sf4 = sf_blocchi_3x3


# Bande orizzontali N=6 (puoi cambiare N)
band_sf2 = band_sf_list$band_sf_6
band_sf3 = band_sf_list$band_sf_18
band_sf5 = band_sf_list$band_sf_9


# Bande verticali N=4 (puoi cambiare N)
colband_sf1 <- colband_sf_list$colband_sf_5 
colband_sf2 <- colband_sf_list$colband_sf_3
...
df_plot <- bind_rows(
  ...
  band_sf3 %>% mutate(layer = "Bande Orizzontali"),
  band_sf5 %>% mutate(layer = "Bande Orizzontali"),
  colband_sf1 %>% mutate(layer = "Bande Verticali"),
  colband_sf2 %>% mutate(layer = "Bande Verticali"),
  blocchi_sf1 %>% mutate(layer = "Blocchi"),
  blocchi_sf2 %>% mutate(layer = "Blocchi"),
  blocchi_sf4 %>% mutate(layer = "Blocchi")
)
...
ggplot(df_plot) +
  ...
  scale_alpha_manual(values = c("TRUE" = 0.55, "FALSE" = 0), guide = "none") +
  theme_void() +
  labs(title = "") +
  theme(
...
# XGBOOST SHAP ----
# shap_really_long <- shap.prep(m.xgb.r, X_train = xmat.model.ver)
# 
# df_shap = as.data.frame(shap_really_long)
# df_shap = df_shap[,c(2,6)]
# df_shap
# names(df_shap) <- c("var", "coeff")
# 
# df_shap <- unique(df_shap)
# dim(df_shap)
# df_shap
# 
# # 1. Oggetto prese (A-K, senza 1 finale)
...
# 
# # 2. mean_rowband
# mean_rowband_vars <- df_shap %>%
#   filter(str_detect(var, "^mean_rowband"))
# 
# # 3. mean_colband
# mean_colband_vars <- df_shap %>%
#   filter(str_detect(var, "^mean_colband"))
# 
# # 4. Blocchi per tipo e solo quelli con "mean"
# blocchi_6x4 <- df_shap %>% filter(str_detect(var, "^blocco6x4.*mean"))
# blocchi_9x4 <- df_shap %>% filter(str_detect(var, "^blocco9x4.*mean"))
# blocchi_6x2 <- df_shap %>% filter(str_detect(var, "^blocco6x2.*mean"))
# blocchi_3x3 <- df_shap %>% filter(str_detect(var, "^blocco3x3.*mean"))
# 
# # Applica la funzione a tutti i tuoi df
# mean_rowband_vars <- clean_vars_base(mean_rowband_vars)
# mean_colband_vars <- clean_vars_base(mean_colband_vars)
...
#   )
#   
#   band_df$mean[is.na(band_df$mean)] <- 0
#   
#   # Crea poligoni
#   band_polys <- lapply(1:nrow(band_df), function(i) {
#     st_polygon(list(matrix(c(
...
# 
# df <- data.frame(
#   blocco = c("blocco3x3_0_0", "blocco3x3_0_3", "blocco3x3_0_6",
#              "blocco3x3_3_0", "blocco3x3_3_3", "blocco3x3_3_6",
#              "blocco3x3_6_0", "blocco3x3_6_3", "blocco3x3_6_6",
#              "blocco3x3_9_0", "blocco3x3_9_3", "blocco3x3_9_6",
#              "blocco3x3_12_0", "blocco3x3_12_3", "blocco3x3_12_6",
#              "blocco3x3_15_0", "blocco3x3_15_3", "blocco3x3_15_6"),
#   mean = c(-9.5397882678, -1.1479400009, -0.7886727088,
#            0.0169962740, -0.3050282473, 0,
#            0, 1.7104213772, -0.2107604564,
#            0, 0.0493756781, 2.6320343904,
#            -0.8534838643, 0, -1.2601731842,
#            -0.0575168581, 0, 0)
# )
# 
# 
# df2 <- df %>%
#   mutate(
#     #blocco = str_extract(feature, "blocco\\d+x\\d+"),
#     X = as.numeric(str_extract(blocco, "\\d+(?=x)")),
#     Y = as.numeric(str_extract(blocco, "(?<=x)\\d+")),
#     # split per estrarre I e J
#     I = as.numeric(sapply(str_split(blocco, "_"), function(x) x[2])),
#     J = as.numeric(sapply(str_split(blocco, "_"), function(x) x[3])),
#     x_min = J+1,
#     x_max = J + X,
#     y_min = I+1,
#     y_max = I + Y
#   ) %>%
#   select(blocco, x_min, x_max, y_min, y_max, mean)
# df2
# 
# blocchi_sf <- lapply(1:nrow(df2), function(i) {
#   st_polygon(list(matrix(c(
...
#   geom_sf(data = grid_sf, aes(fill = coeff), color = "grey80") +
#   scale_fill_viridis_c(
#     option = "magma", direction = -1, na.value = "grey95", name = "Coefficiente Prese"
#   ) +
#   theme_void() +
#   labs(title = "Prese colorate in base al coefficiente") +
#   theme(
#     legend.position = "right",  # legenda a destra
...
#       title.position = "top",
#       title.hjust = 0.5,
#       barwidth = 1,   # stringe la barra (orizzontalmente)
#       barheight = 7     # accorcia la barra (verticalmente)
#     )
#   ) +
#   geom_text(
...
#   )
# p1
# # 2. Bande orizzontali (proiettate sulla griglia) ---
# for(n in names(band_sf_list)) {
#   p <- ggplot() +
#     geom_sf(data = grid_sf, fill = "grey95", color = "grey80") +
#     geom_sf(data = band_sf_list[[n]], aes(fill = mean), color = NA, alpha = 0.3) +
#     scale_fill_viridis_c(
#       option = "plasma", direction = -1, na.value = "grey95", name = paste("Media bande orizzontali (N =", gsub("band_sf_", "", n), ")")
#     ) +
#     theme_void() +
#     labs(title = paste("Bande orizzontali (righe) - N =", gsub("band_sf_", "", n))) +
#     theme(
...
#       legend.margin = margin(0, 4, 0, 4),
#       plot.title.position = "plot",
#       plot.title = element_text(size = 15, hjust = 0.2, margin = margin(b = 2.5))
#     ) +
#     guides(
#       fill = guide_colourbar(
#         title.position = "top",
...
# }
# 
# # 3. Bande verticali (proiettate sulla griglia) ---
# for(n in names(colband_sf_list)) {
#   p <- ggplot() +
#     geom_sf(data = grid_sf, fill = "grey95", color = "grey80") +
#     geom_sf(data = colband_sf_list[[n]], aes(fill = mean), color = NA, alpha = 0.5) +
#     scale_fill_viridis_c(
#       option = "inferno", direction = -1, na.value = "grey95", name = paste("Media bande verticali (N =", gsub("colband_sf_", "", n), ")")
#     ) +
#     theme_void() +
#     labs(title = paste("Bande verticali (colonne) - N =", gsub("colband_sf_", "", n))) +
#     theme(
...
# }
# 
# # 4. Blocchi (macro-zone) proiettati sulla griglia ---
# pal <- "cividis"   # oppure "magma", "plasma", "inferno"
# fill_lab <- "Media blocchi"
# title <- "Blocchi macro-zone"
# 
# # Blocchi 3x3
# p_blocchi_3x3 <- plot_blocchi(sf_blocchi_3x3, grid_sf, df_col, df_row,
#                               title = "Blocchi 3x3", fill_lab = "Media blocchi 3x3", pal = "cividis")
# print(p_blocchi_3x3)
# 
# # Blocchi 6x4
# p_blocchi_6x4 <- plot_blocchi(sf_blocchi_6x4, grid_sf, df_col, df_row,
#                               title = "Blocchi 6x4", fill_lab = "Media blocchi 6x4", pal = "plasma")
# print(p_blocchi_6x4)
# 
# # Blocchi 6x2
# p_blocchi_6x2 <- plot_blocchi(sf_blocchi_6x2, grid_sf, df_col, df_row,
#                               title = "Blocchi 6x2", fill_lab = "Media blocchi 6x2", pal = "magma")
# print(p_blocchi_6x2)
# 
# # Blocchi 9x4
# p_blocchi_9x4 <- plot_blocchi(sf_blocchi_9x4, grid_sf, df_col, df_row,
#                               title = "Blocchi 9x4", fill_lab = "Media blocchi 9x4", pal = "inferno")
# print(p_blocchi_9x4)
# 
# 
# ### VISUALIZZAZIONE INTEGRATA ----
...
# band_sf <- band_sf_list$band_sf_6     # Bande orizzontali N=6 (puoi cambiare N)
# colband_sf <- colband_sf_list$colband_sf_4  # Bande verticali N=4 (puoi cambiare N)
# 
# ggplot() +
#   geom_sf(data = grid_sf, aes(fill = coeff), color = "grey80") +
#   scale_fill_viridis_c(option = "magma", direction = -1, na.value="grey95", name = "Coefficiente Prese") +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = band_sf, aes(fill = mean), alpha = 0.2, color = NA) +
#   scale_fill_viridis_c(option = "plasma", direction = -1, na.value="grey95", name = "Media Bande Orizzontali") +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = colband_sf, aes(fill = mean), alpha = 0.2, color = NA) +
#   scale_fill_viridis_c(option = "inferno", direction = -1, na.value="grey95", name = "Media Bande Verticali") +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = blocchi_sf, aes(fill = mean), alpha = 0.2, color = "purple", size = 1.2) +
#   scale_fill_viridis_c(option = "cividis", direction = -1, na.value="grey95", name = "Media Blocchi") +
#   theme_void() +
#   labs(title="Prese, bande e blocchi colorati") +
#   theme(
#     legend.position = "bottom",
#     legend.title = element_text(size = 12),
#     legend.text = element_text(size = 10)
