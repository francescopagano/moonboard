...
colnames(coef_df)[1] <- "coeff"
coef_df

# Crea una colonna 'var' dai rownames
coef_df <- coef_df %>%
...
# 2. mean_rowband
mean_rowband_vars <- coef_df %>%
  filter(str_detect(var, "^mean_rowband"))

# 3. mean_colband
mean_colband_vars <- coef_df %>%
  filter(str_detect(var, "^mean_colband"))

# 4. Blocchi per tipo e solo quelli con "mean"
blocchi_6x4 <- coef_df %>% filter(str_detect(var, "^blocco6x4.*mean"))
blocchi_9x4 <- coef_df %>% filter(str_detect(var, "^blocco9x4.*mean"))
blocchi_6x2 <- coef_df %>% filter(str_detect(var, "^blocco6x2.*mean"))
blocchi_3x3 <- coef_df %>% filter(str_detect(var, "^blocco3x3.*mean"))

blocchi_6x3 <- coef_df %>% filter(str_detect(var, "^blocco6x3.*mean"))
blocchi_9x2 <- coef_df %>% filter(str_detect(var, "^blocco9x2.*mean"))
...
q_low  <- quantile(coef_df$coeff, 0.20)
q_high <- quantile(coef_df$coeff, 0.80)
...
# prese$coeff[prese$coeff >= -0.01 & prese$coeff <= 0.01] <- NA
# mean_rowband_vars$coeff[mean_rowband_vars$coeff >= -0.01 & mean_rowband_vars$coeff <= 0.01] <- NA
# mean_colband_vars$coeff[mean_colband_vars$coeff >= -0.01 & mean_colband_vars$coeff <= 0.01] <- NA
# 
# blocchi_6x4$coeff[blocchi_6x4$coeff >= -0.01 & blocchi_6x4$coeff <= 0.01] <- NA
# blocchi_9x4$coeff[blocchi_9x4$coeff >= -0.01 & blocchi_9x4$coeff <= 0.01] <- NA
# blocchi_6x2$coeff[blocchi_6x2$coeff >= -0.01 & blocchi_6x2$coeff <= 0.01] <- NA
# blocchi_3x3$coeff[blocchi_3x3$coeff >= -0.01 & blocchi_3x3$coeff <= 0.01] <- NA
# blocchi_6x3$coeff[blocchi_6x3$coeff >= -0.01 & blocchi_6x3$coeff <= 0.01] <- NA
# blocchi_9x2$coeff[blocchi_9x2$coeff >= -0.01 & blocchi_9x2$coeff <= 0.01] <- NA
...
p1 <- ggplot() +
  ...
  theme_void() +
  labs(title = "Intensità coefficienti delle prese - Elastic Net") +
  theme(
    legend.position = "right",  # legenda a destra
    legend.title = element_text(size = 10),
    legend.text = element_text(size = 10),
...
p_blocchi_3x3 <- plot_blocchi(sf_blocchi_3x3, grid_sf, df_col, df_row,
                              title = "Blocchi 3x3 - Elastic Net", fill_lab = "Coefficiente blocchi 3x3", sub_pal)
print(p_blocchi_3x3)

# Blocchi 6x4
p_blocchi_6x4 <- plot_blocchi(sf_blocchi_6x4, grid_sf, df_col, df_row,
                              title = "Blocchi 6x4 - Elastic Net", fill_lab = "Coefficiente blocchi 6x4", sub_pal)
print(p_blocchi_6x4)

# Blocchi 6x2
p_blocchi_6x2 <- plot_blocchi(sf_blocchi_6x2, grid_sf, df_col, df_row,
                              title = "Blocchi 6x2 - Elastic Net", fill_lab = "Coefficiente blocchi 6x2", sub_pal)
print(p_blocchi_6x2)

# Blocchi 9x4
p_blocchi_9x4 <- plot_blocchi(sf_blocchi_9x4, grid_sf, df_col, df_row,
                              title = "Blocchi 9x4 - Elastic Net", fill_lab = "Coefficiente blocchi 9x4", sub_pal)
print(p_blocchi_9x4)

# Blocchi 6x2
p_blocchi_6x3 <- plot_blocchi(sf_blocchi_6x3, grid_sf, df_col, df_row,
                              title = "Blocchi 6x3 - Elastic Net", fill_lab = "Coefficiente blocchi 6x3", sub_pal)
print(p_blocchi_6x3)

# Blocchi 9x2
p_blocchi_9x2 <- plot_blocchi(sf_blocchi_9x2, grid_sf, df_col, df_row,
                              title = "Blocchi 9x2 - Elastic Net", fill_lab = "Coefficiente blocchi 9x2", sub_pal)
print(p_blocchi_9x2)


### VISUALIZZAZIONE INTEGRATA ----

blocchi_sf1 = sf_blocchi_3x3          # Blocchi 3x3 (puoi cambiare con sf_blocchi_6x4, ecc)
blocchi_sf2 = sf_blocchi_9x2

# Bande orizzontali N=6 (puoi cambiare N)
band_sf1 <- band_sf_list$band_sf_18
band_sf2 = band_sf_list$band_sf_4
band_sf3 = band_sf_list$band_sf_3
band_sf4 = band_sf_list$band_sf_2

colband_sf1 <- colband_sf_list$colband_sf_11 # Bande verticali N=4 (puoi cambiare N)
colband_sf2 = colband_sf_list$colband_sf_4
...
#   
#   geom_sf(data = band_sf1, aes(fill = mean), alpha = 0.3, color = NA) +
...
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = band_sf2, aes(fill = mean), alpha = 0.3, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value =  "transparent", name = "Coefficiente Bande Orizzontali")+
#   #scale_fill_viridis_c(option = "rocket", direction = -1, na.value = "grey95", name = "Coefficiente Bande Orizzontali") +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = band_sf3, aes(fill = mean), alpha = 0.3, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value =  "transparent", name = "Coefficiente Bande Orizzontali")+
#   #scale_fill_viridis_c(option = "rocket", direction = -1, na.value = "grey95", name = "Coefficiente Bande Orizzontali") +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = band_sf4, aes(fill = mean), alpha = 0.3, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value =  "transparent", name = "Coefficiente Bande Orizzontali")+
#   #scale_fill_viridis_c(option = "rocket", direction = -1, na.value = "grey95", name = "Coefficiente Bande Orizzontali") +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = colband_sf1, aes(fill = mean), alpha = 0.3, color = NA) +
...
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = colband_sf2, aes(fill = mean), alpha = 0.3, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value =  "transparent", name = "Coefficiente Bande Verticali")+
#   #scale_fill_viridis_c(option = "rocket", direction = -1, na.value = "grey95", name = "Coefficiente Bande Verticali") +
#   ggnewscale::new_scale_fill() +
#   
...
# library(ggnewscale)
# 
# # Range unico per i fill (come suggerito prima)
# all_means <- c(
#   grid_sf$coeff,
#   band_sf1$mean, band_sf2$mean, band_sf3$mean, band_sf4$mean,
#   colband_sf1$mean, colband_sf2$mean,
#   blocchi_sf1$mean, blocchi_sf2$mean
# )
# range_means <- range(all_means, na.rm = TRUE)
# 
# ggplot() +
#   # Primo layer: no alpha manuale, solo fill
#   # geom_sf(data = grid_sf, aes(fill = coeff), color = "grey80", alpha = 1) +
#   # scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Coefficiente Prese", limits = range_means) +
#   # ggnewscale::new_scale_fill() +
#   
#   # Layer bande orizzontali
#   geom_sf(data = band_sf1, aes(fill = mean), alpha = 0.5, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Bande Orizzontali", limits = range_means) +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = band_sf2, aes(fill = mean), alpha = 0.5, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Bande Orizzontali", limits = range_means) +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = band_sf3, aes(fill = mean), alpha = 0.5, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Bande Orizzontali", limits = range_means) +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = band_sf4, aes(fill = mean), alpha = 0.5, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Bande Orizzontali", limits = range_means) +
#   ggnewscale::new_scale_fill() +
#   
#   # Layer bande verticali
#   geom_sf(data = colband_sf1, aes(fill = mean), alpha = 0.5, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Bande Verticali", limits = range_means) +
#   ggnewscale::new_scale_fill() +
#   
#   geom_sf(data = colband_sf2, aes(fill = mean), alpha = 0.5, color = NA) +
#   scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Bande Verticali", limits = range_means) +
#   ggnewscale::new_scale_fill() +
#   
#   # Blocchi con alpha manuale
#   geom_sf(data = blocchi_sf1, aes(fill = mean, alpha = !is.na(mean)), color = NA, size = 1.2) +
#   scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Blocchi", limits = range_means) +
#   scale_alpha_manual(values = c("TRUE" = 0.5, "FALSE" = 0), guide = "none") +
#   ggnewscale::new_scale_fill() +
#   ggnewscale::new_scale("alpha") +  # <<<< AGGIUNGI QUESTA RIGA
#   
#   geom_sf(data = blocchi_sf2, aes(fill = mean, alpha = !is.na(mean)), color = NA, size = 1.2) +
#   scale_fill_gradientn(colors = sub_pal, na.value = "transparent", name = "Blocchi", limits = range_means) +
#   scale_alpha_manual(values = c("TRUE" = 0.5, "FALSE" = 0), guide = "none") +
#   
#   theme_void() +
...
df_plot <- bind_rows(
  ...
  band_sf4 %>% mutate(layer = "Bande Orizzontali"),
